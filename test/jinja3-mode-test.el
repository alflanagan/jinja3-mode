...
(describe
 ...
 (it
  "returns the built-in closing keywords"
  (let ((jinja3-user-keywords nil))
    (expect
     (jinja3-closing-keywords)
     :to-equal
     '("autoescape"
       "block"
       "call"
 ...
 (it
  "adds else and elif to the indenting keywords"
  (let ((jinja3-user-keywords nil))
 ...
 (it
  "prepends jinja3-user-functions to the filter functions"
...
(describe
 ...
 (it
  "finds the nearest unclosed block tag"
  (jinja3-test--in-buffer "{% for a in b %}\n{% if c %}\n"
    (goto-char (point-max))
    (expect (jinja3-find-open-tag) :to-equal '("if" " c"))))

 (it
  "returns nil when there is no open tag"
...
(describe
 ...
 (it
  "inserts a matching end tag for a simple block"
 ...
 (it
  "echoes the block name when closing a block tag"
 ...
 (it
  "signals an error when there is nothing to close"
...
(describe
 ...
 (it
  "inserts an empty statement tag with point inside"
 ...
 (it
  "inserts an empty variable tag with point inside"
  (jinja3-test--in-buffer ""
    (jinja3-insert-var)
    (expect (buffer-string) :to-equal "{{  }}")
    (expect (point) :to-equal 4)))
 ...
 (it
  "inserts an empty comment tag with point inside"
  (jinja3-test--in-buffer ""
...
(describe
 ...
 (it
  "indents the body of a for block"
  (expect
   (jinja3-test--reindent "{% for x in y %}\nhello\n{% endfor %}\n")
   :to-equal "{% for x in y %}\n  hello\n{% endfor %}\n"))

 (it
  "dedents else and re-indents both branches"
  (expect
   (jinja3-test--reindent "{% if a %}\nx\n{% else %}\ny\n{% endif %}\n")
   :to-equal "{% if a %}\n  x\n{% else %}\n  y\n{% endif %}\n"))

 (it
  "indents nested blocks cumulatively"
  (expect
   (jinja3-test--reindent
    "{% for a in b %}\n{% if c %}\nx\n{% endif %}\n{% endfor %}\n")
   :to-equal
   (concat
    "{% for a in b %}\n"
 ...
 (it
  "leaves an already-correct buffer unchanged"
  (let ((text "{% for x in y %}\n  hello\n{% endfor %}\n"))
...
(describe
 ...
 (it
  "highlights variable names inside {{ }}"
  (expect
   (jinja3-test--has-face
    'font-lock-variable-name-face (jinja3-test--face-of "{{ name }}" "name"))
   :to-be-truthy))

 (it
  "highlights block keywords inside {% %}"
  (expect
   (jinja3-test--has-face
    'font-lock-keyword-face (jinja3-test--face-of "{% if x %}" "if"))
   :to-be-truthy))

 (it
  "highlights comment bodies inside {# #}"
  (expect
   (jinja3-test--has-face
    'font-lock-comment-face (jinja3-test--face-of "{# a note #}" "a note"))
   :to-be-truthy))

 (it
  "highlights builtin keywords"
  (expect
   (jinja3-test--has-face
    'font-lock-builtin-face (jinja3-test--face-of "{% extends x %}" "extends"))
   :to-be-truthy))

 (it
  "highlights builtin filter functions"
  (expect
   (jinja3-test--has-face
...
(describe
 ...
 (it
  "uses Jinja3 comment delimiters"
  (jinja3-test--in-buffer ""
    (expect comment-start :to-equal "{#")
    (expect comment-end :to-equal "#}")))

 (it
  "installs jinja3-indent-line as the indent function"
  (jinja3-test--in-buffer ""
    (expect indent-line-function :to-be 'jinja3-indent-line)))

 (it
  "binds the tag editing commands in the mode map"
  (expect (lookup-key jinja3-mode-map (kbd "C-c c")) :to-be 'jinja3-close-tag)
  (expect (lookup-key jinja3-mode-map (kbd "C-c t")) :to-be 'jinja3-insert-tag)
  (expect (lookup-key jinja3-mode-map (kbd "C-c v")) :to-be 'jinja3-insert-var)
  (expect
   (lookup-key jinja3-mode-map (kbd "C-c #"))
   :to-be 'jinja3-insert-comment))

 (it
  "registers the .j2 and .jinja file extensions"
  (expect (cdr (assoc "\\.j2\\'" auto-mode-alist)) :to-be 'jinja3-mode)
  (expect (cdr (assoc "\\.jinja\\'" auto-mode-alist)) :to-be 'jinja3-mode)))
